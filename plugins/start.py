    ...
    if len(text) > 7:
        ...
        if len(argument) == 3:
            ...
            try:
                ...
                return
        temp_msg = await message.reply("<code>pelan-pelan sayang...</code>")
        try:
            messages = await get_messages(client, ids)
        except BaseException:
            await message.reply_text("<b>maaf error </b>")
            return
        ...
        buttons = [
            [InlineKeyboardButton("HALO", callback_data="about")],
            [
...
async def get_users(client: Bot, message: Message):
    ...
    await msg.edit(f"{len(users)} <b>BOT PRIVATE, GADA PENGGUNA LAIN</b>")
